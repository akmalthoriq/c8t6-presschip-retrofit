...
void System_Run_Cycle(void) {
    ...
    switch (CurrentState) {
        ...
        // Logika Auto Cycle (cuman berjalan jika auto_cycle_active == 1)
        case STATE_HOMING:
            if (!auto_cycle_active) break;
            vertikal_naik();

            if(prox_horiz_close == PROXY_INAKTIF){
            	horizontal_maju(); //untuk nutup pembuangan terlebih dahulu
            }
            ...
            if (prox_vert_homing == PROXY_AKTIF && prox_vert_middle == PROXY_INAKTIF && prox_vert_eject == PROXY_AKTIF) {
                vert_press_counter++;
                CurrentState = STATE_AUGER_FILLING; // <--- Ganti dari STATE_DELAY_MATERIAL_IN
                stop_all_actuators();
                auger_maju(); // Jalankan motor pengisi
        ...
        case STATE_VERT_PRESS_BY_GAUGE:
            ...
            if (CurrentState != STATE_VERT_PRESS_BY_GAUGE || gauge_press_timer_start == 0) {
            ...
            if (HAL_GetTick() - gauge_press_timer_start > PRESS_GAUGE_TIMEOUT_MS) {
                    ...
                    Send_VCP_Message("CRITICAL ERROR: PRESS GAUGE TIMEOUT! Gagal mencapai 150 Bar dalam 30s.");
            ...
            if (current_pressure_adc >= target_pressure_adc) {
            	...
            	stop_all_actuators();
            	Send_VCP_Message("PRESS GAUGE SELESAI: 150 Bar tercapai. Naik By Timer");
        ...
        case STATE_VERT_UP_BY_TIMER:
        	...
        	if (HAL_GetTick() - vert_up_timer_start >= vert_up_duration_ms){
        		...
        		stop_all_actuators();
        		Send_VCP_Message("Silinder vertikal naik selama timer selesai. Menuju membuka pembuangan");
        		state_timer_start = HAL_GetTick();
        	}
        	break;
        ...
        case STATE_VERT_EJECT:
                   ...
                   if (prox_vert_homing == PROXY_INAKTIF && prox_vert_middle == PROXY_AKTIF && prox_vert_eject == PROXY_INAKTIF) { //Logika proxy_vert_middle terbalik, PROXY_AKTIF -> artinya Sensor OFF
                       CurrentState = STATE_UP_TO_PROX_VERT_MIDDLE;
                       stop_all_actuators();
                       Send_VCP_Message("Mendorong material keluar selesai. Kembali naik sampai Proxy vert middle aktif terlebih dahulu.");
                       state_timer_start = HAL_GetTick();
                   }
                   break;
        ...
        case STATE_UP_TO_PROX_VERT_MIDDLE:
			if(!auto_cycle_active) break;
				vertikal_naik();
				if (prox_vert_homing == PROXY_INAKTIF && prox_vert_middle == PROXY_INAKTIF && prox_vert_eject == PROXY_AKTIF){ // logika proxy_vert_middle kebalik
					CurrentState =  STATE_HORIZ_CLOSE;
					stop_all_actuators();
					Send_VCP_Message("Naik sampai proxy press nyala sudah selesai. Delay 1,5 detik sebelum menutup pembuangan");
        ...
        case STATE_HORIZ_CLOSE:
        	...
        	if(HAL_GetTick() - delay_after_naik_timer_start >= delay_after_naik_ms){
        		horizontal_maju(); // Menutup pembuangan
			}

        	if (prox_horiz_close == PROXY_AKTIF && prox_horiz_open == PROXY_INAKTIF){
        		CurrentState = STATE_CYCLE_END_TO_HOME;
        		Send_VCP_Message("Pembuangan berhasil ditutup, kembali ke homing");
        		state_timer_start = HAL_GetTick();
        	}
        	break;

        case STATE_CYCLE_END_TO_HOME:
            if (!auto_cycle_active) break;
            CurrentState = STATE_HOMING; // Lanjut ke siklus berikutnya
            Send_VCP_Message("Siklus Selesai. Mengulang Homing.");
            state_timer_start = HAL_GetTick();
            break;
    }
 */
